...
public class FTPUtil {
     ...
     * @return 成功返回true，否则返回false
     */
    public List<String> downloadFilesNew(String ftpPath, String savePath) {
        ...
        if (ftpClient != null) {
            ...
            for1:for (String ff : fs) {
                    if(".".equals(ff)||("..".equals(ff))){
                        continue;
                    }
                    //如果本地已存在的文件则不下载
                    File fileLocal = new File(savePath);
                    String[] filelist = fileLocal.list();
                    String ftpName = new String(ff.getBytes(serverCharset), localCharset);
                    for(String temp:filelist) {
                        if(ftpName.equals(temp)) {
                            continue for1;
                        }
                    }
                    File file = new File(savePath + File.separator + ftpName);
                    fileNames.add(ftpName);
                    try (OutputStream os = new FileOutputStream(file)) {
                        ftpClient.retrieveFile(ff, os);
                    } catch (Exception e) {
                        log.error(e.getMessage(), e);
...
//        Map<String,Object> result = fTPUtil.downLoadTableFile("中文test","G:\\FtpTest");
//        List<FTPUtil> ftpUtilNew = new ArrayList<FTPUtil>();
//        ftpUtilNew.add(fTPUtil);
//        List list = ftpUtilNew;
//        List<FTPUtil> ftpUtils = (List<FTPUtil>)list;
//        FTPUtil ftpUtil = ftpUtils.get(0);
//        List<? extends Object> objects = ftpUtilNew ;
//        List<Object> objects = new ArrayList<Object>();
//        List<FTPUtil> test = (List<FTPUtil>)objects;
    }
}